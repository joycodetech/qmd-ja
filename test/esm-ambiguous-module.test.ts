...
describe("Node ESM entrypoints", () => {
  ...
  test("the build stamps the commit it was built from", () => {
    ...
    // qmd-ja: `qmd-ja 2.6.3-ja.1` or `qmd-ja 2.6.3-ja.1 (abc1234)` /
    // `(abc1234-dirty)` — never anything else. Upstream's own binary/version
    // format (`qmd 2.6.3`) is also accepted for compatibility with the
    // unmodified regression this test was ported from (#787).
    expect(output).toMatch(/^qmd(?:-ja)? \d+\.\d+\.\d+(?:-ja\.\d+)?(?: \([0-9a-f]{7,}(?:-dirty)?\))?$/);
